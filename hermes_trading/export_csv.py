...
def export_strategy():
    ...
    with open(dst, "w", newline="", encoding="utf-8") as f:
        ...
        w.writerow(["Stop Loss",        f"{s.get('stop_loss_pct', '?')} pct", "Exit if loss exceeds this"])
        w.writerow(["Take Profit",      f"{s.get('take_profit_pct', '?')} pct", "Exit if gain exceeds this"])
        w.writerow(["Short Threshold",  entry.get("short_threshold", 70),  "Enter short when RSI above this"])
